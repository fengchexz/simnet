...
'''Aligning Module'''
class AligningLayer(nn.Module):
    def __init__(self, d_model, d_inner, n_head, d_k, d_v, dropout=0.1):
        super(AligningLayer, self).__init__()
        self.enc_attn = MultiHeadAttention( n_head, d_model, d_k, d_v, dropout=dropout )
        self.pos_ffn = PositionwiseFeedForward( d_model, d_inner, dropout=dropout )
        self.layer_norm = nn.LayerNorm( d_model )
    '''T和V互换为不同的Aligning操作'''
    def forward(self, T, V):
        Refine_VT = self.enc_attn( T, V, V )
        Refine_VT = self.pos_ffn( Refine_VT )
        return Refine_VT
...
'''Integrating Module'''
class IntegratingLayer(nn.Module):
    def __init__(self, d_model, d_inner, n_head, d_k, d_v, dropout=0.1):
        super(IntegratingLayer, self).__init__()
        self.enc_attn = MultiHeadAttention( n_head, d_model, d_k, d_v, dropout=dropout )
        self.pos_ffn = PositionwiseFeedForward( d_model, d_inner, dropout=dropout )
    '''T和V互换为不同的Integrating操作'''
    def forward(self,T):
        Refine_T = self.enc_attn( T, T, T )
